...
def remove_warnings():
    ...
    # This will also suppress the PerformanceWarning you asked about
...
# %% [markdown]
# ## Load data
# The functions below have been optimized to avoid DataFrame fragmentation.
...
def apply_contaminations_from_files(contamination_files, df, n_points):
    ...
    df_final = pd.concat(df_list, ignore_index=True).copy() # .copy() ensures a de-fragmented frame
...
# %% [markdown]
# ## Clean data
# Your clean data loading functions remain the same.
...
def generate_df_with_noise_std(df, n_repeat, noise_std, seed=None):
    ...
    if isinstance(noise_std, (int, float)):
        noise_replicated = np.full(df_spectra_replicated.shape, noise_std, dtype='float32')
    else:
    ...
    gaussian_noise = np.random.normal(0, noise_replicated, df_spectra_replicated.shape).astype('float32')
    ...
    new_cols_data = {
        "noise_std": np.repeat(noise_std, n_repeat) if isinstance(noise_std, (list, np.ndarray, pd.Series)) else noise_std,
        "n_repeat": n_repeat
    }
...
for snr in snr_values:
    ...
    temp_CO2_data = generate_df_with_noise_std(df=CO2_data, n_repeat=5000, noise_std=noise)
    temp_CH4_data = generate_df_with_noise_std(df=CH4_data, n_repeat=500, noise_std=noise)
    temp_airless_data = generate_df_with_noise_std(df=airless_data, n_repeat=5000, noise_std=noise)

    current_noisy_df = pd.concat([temp_CO2_data, temp_CH4_data, temp_airless_data], ignore_index=True)
    ...
    del temp_CO2_data, temp_CH4_data, temp_airless_data, current_noisy_df, normalized_data
    gc.collect()

    # --- Generate Corresponding Clean Data (Uncontaminated + No Noise) ---
    temp_no_CO2_data = generate_df_with_noise_std(df=CO2_data_clean, n_repeat=5000, noise_std=0)
    temp_no_CH4_data = generate_df_with_noise_std(df=CH4_data_clean, n_repeat=500, noise_std=0)
    temp_no_airless_data = generate_df_with_noise_std(df=airless_data_clean, n_repeat=5000, noise_std=0)

    current_clean_df = pd.concat([temp_no_CO2_data, temp_no_CH4_data, temp_no_airless_data], ignore_index=True)
    ...
    del temp_no_CO2_data, temp_no_CH4_data, temp_no_airless_data, current_clean_df, normalized_data
    gc.collect()
...
for idx in indices:
    ...
    plt.plot(waves, X_test_noisy[idx].flatten(), label="Noisy Input Spectrum", alpha=0.5)
    plt.plot(waves, decoded_spectra[idx].flatten(), label="Denoised (Reconstructed) Spectrum", linestyle='--')
